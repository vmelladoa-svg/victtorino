"""
Productos COMPLEMENTARIOS de baño y cocina (venta cruzada con la línea de Victor):
grifería, kits de ducha, sifones, escurridores, organizadores, accesorios.
Agrega hojas al xlsx y genera ficha visual alila_fichas_complementarios.html
"""
...
comp = al[(al["Estado"] == "上线销售") & mkt.notna() &
          cat.str.contains(r"Cocina y Menaje|Hogar y Muebles>Baños|Mobiliario para Baños|Mobiliario para Cocinas")].copy()
comp["Nombre ES"] = comp.apply(lambda r: slug(r["Link MercadoLibre"]) or r.get("Nombre (EN)") or "(sin nombre)", axis=1)
...
cols = ["Código", "Nombre ES", "Categoría ES", "N° ofertas ML", "Stock alila", "Costo 1688 CLP",
        "Precio alila CLP", "Precio mercado ML", "Margen vs alila CLP", "Margen vs alila %", "Margen vs 1688 %",
        "Link proveedor (1688)", "Link MercadoLibre", "N° fotos", "Foto principal"]
comp = comp[[c for c in cols if c in comp.columns]]
comp["_s"] = comp["Stock alila"].fillna(-1)
reco = comp[(comp["N° ofertas ML"] <= 10) & (comp["Margen vs alila %"] >= 45) &
            (comp["Margen vs alila CLP"] >= 3000) & (comp["_s"] > 0) &
            (~comp["Nombre ES"].isin(["(sin nombre)", "Nan", "1"]))].sort_values("Margen vs alila %", ascending=False).drop(columns="_s")
todo = comp.drop(columns="_s").sort_values("Margen vs alila CLP", ascending=False)

sheets = pd.read_excel(XLS, sheet_name=None)
sheets["COMPLEMENTARIOS_RECO"] = reco
sheets["COMPLEMENTARIOS_TODO"] = todo
...
for i, (_, r) in enumerate(ficha.iterrows(), 1):
    ...
    stag = ('<span class="tag">stock: s/d</span>' if s is None else
            '<span class="tag out">⚠ AGOTADO</span>' if s <= 0 else
            f'<span class="tag mid">stock: {s} u (bajo)</span>' if s < 20 else
            f'<span class="tag low">stock: {s} u</span>')
    cards.append(f"""
    <div class="card"><div class="rank">#{i}</div><div class="ph">{img_html}</div>
      <div class="info"><h2>{r['Nombre ES']}</h2><div class="cat">{r.get('Categoría ES','')} · cód {cod}</div>
        ...
        <div class="meta">{stag}<span class="tag {'low' if r['N° ofertas ML']<=3 else 'mid'}">{int(r['N° ofertas ML'])} competidores en ML</span><span class="tag alt">Margen 1688 directo: {r['Margen vs 1688 %']}%</span></div>
        <div class="btns">{b1688}{bml}</div></div></div>""")

html = f"""<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>Complementarios baño y cocina — alila</title>
...
<h1>Complementarios de baño y cocina — alila</h1>
<p class="sub">Accesorios para venta cruzada con tu línea (grifería, kits ducha, sifones, escurridores, organizadores). Baja competencia + margen sano + stock. CNY→CLP 130,9.</p>
{''.join(cards)}</body></html>"""
out = ROOT / "alila_fichas_complementarios.html"
out.write_text(html, encoding="utf-8")
print(f"Complementarios baño/cocina con precio: {len(comp)} | RECOMENDADOS (filtrados): {len(reco)}")
print(f"Ficha: {out.name} ({round(out.stat().st_size/1024)} KB, {len(ficha)} fichas)")
print("\nRecomendados (top por margen CLP):")
print(reco.head(15)[["Nombre ES", "Categoría ES", "N° ofertas ML", "Stock alila", "Precio alila CLP", "Precio mercado ML", "Margen vs alila %"]].to_string(index=False))
