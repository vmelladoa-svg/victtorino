"""
Agrega sección FAQ al final de la descripción de cada una de las 10 categorías
principales. 5 preguntas + respuestas por categoría, en formato H3+P semántico
para extracción por IA. Conserva la descripción premium que ya tienen.
"""
import json
import sys
import io
...
FAQS = {
    ...
    118: {  # Shower/Mamparas/Receptaculos
        "name": "Shower/Mamparas/Receptáculos",
        "faqs": [
...
def faqs_html(faqs):
    ...
    for pregunta, respuesta in faqs:
        parts.append(f"<h3>{pregunta}</h3>")
        parts.append(f"<p>{respuesta}</p>")
    return "\n".join(parts)


# Aplicar a cada categoría
print(f"Agregando sección FAQ a {len(FAQS)} categorías...\n")
ok = 0
for cat_id, data in FAQS.items():
    # Leer descripción actual
    r = requests.get(f"{WC}/wp-json/wc/v3/products/categories/{cat_id}", params=P, timeout=30)
    actual = r.json().get("description", "")
    # Verificar si ya tiene la sección "Preguntas frecuentes"
    if "Preguntas frecuentes" in actual:
        print(f"  cat {cat_id} '{data['name']}' YA tiene FAQ, salto")
        continue
    nueva = actual + "\n\n" + faqs_html(data["faqs"])
    ...
    for n in range(1, 4):
        try:
            ...
            d = r2.json()
            import re
            plain = re.sub(r"<[^>]+>", " ", d.get("description", ""))
            print(f"  cat {cat_id:3} '{data['name']:32}' OK -> {len(plain.split())} palabras totales")
            ok += 1
            break
        except Exception as e:
            print(f"  cat {cat_id} {type(e).__name__}; retry")
            time.sleep(4 * n)
    time.sleep(1)

print(f"\n{ok}/{len(FAQS)} categorías con FAQ agregada")
